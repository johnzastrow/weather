...
### 2017 
my_query2017 = ''' SELECT `WEEKW`, `w_temp_f_avg` FROM `weather`.`v_kpwm_weekly` WHERE (YEARY = 2017);'''
...
### 2018 
my_query2018 = ''' SELECT `WEEKW`, `w_temp_f_avg` FROM `weather`.`v_kpwm_weekly` WHERE (YEARY = 2018);'''
...
### 2019 
my_query2019 = ''' SELECT `WEEKW`, `w_temp_f_avg` FROM `weather`.`v_kpwm_weekly` WHERE (YEARY = 2019);'''
...
### 2019 
my_max = ''' SELECT max(WEEKW) as maxweek FROM `weather`.`v_kpwm_weekly` WHERE (YEARY = 2019);'''
...
plt.minorticks_on()
plt.title("Weekly Air Temps (F) by Week Number of Year from Station kpwm on: "+ today)
