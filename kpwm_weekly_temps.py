...
### 2017 
my_query2017 = ''' SELECT `WEEKW`, `w_temp_f_avg` FROM `weather`.`v_KPWM_weekly` WHERE (YEARY = 2017) and WEEKW <= 52;'''
...
### 2020 
my_query2020 = ''' SELECT `WEEKW`, `w_temp_f_avg` FROM `weather`.`v_KPWM_weekly` WHERE (YEARY = 2020) and WEEKW <= 52;'''
...
### 2021 
my_query2021 = ''' SELECT `WEEKW`, `w_temp_f_avg` FROM `weather`.`v_KPWM_weekly` WHERE (YEARY = 2021) and WEEKW <= 52;'''
...
### Max  
my_max = ''' SELECT max(WEEKW) as maxweek FROM `weather`.`v_KPWM_weekly` WHERE (YEARY = ''' + todayyear + ''');'''
...
plt.minorticks_on()
plt.title("Weekly Average Air Temps (F) by Week Number of Year from Station kpwm on: "+ today)
...
plt.savefig('kpwmweekly_temps_compare_years.png', dpi=200)
plt.show()
plt.close()
