...
# House electric
my_query_elec = ''' SELECT `d_local`, `d_kwh` FROM `v_daily_electric`  WHERE DATE(d_local) BETWEEN CURDATE() - INTERVAL 60 DAY AND CURDATE(); '''
...
ax.errorbar(dater, tempf_vals, yerr = y_error, markersize=4, fmt='-o', ecolor='#9cb9e5', label="E1248 Temp (avg, min.max)", color='#1269ea', elinewidth=4)
# set x-axis label
