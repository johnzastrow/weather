...
### 2017 
my_query2017 = ''' SELECT `WEEKW`, `w_temp_f_avg` FROM `weather`.`v_E1248_weekly` WHERE (YEARY = 2017) and WEEKW <= 52;'''
...
### 2021 
my_query2021 = ''' SELECT `WEEKW`, `w_temp_f_avg` FROM `weather`.`v_E1248_weekly` WHERE (YEARY = 2021) and WEEKW <= 52;'''
...
### Max  
my_max = ''' SELECT max(WEEKW) as maxweek FROM `weather`.`v_E1248_weekly` WHERE (YEARY = ''' + todayyear + ''');'''
print(my_max)
...
plt.minorticks_on()
plt.title("Weekly Average Air Temps (F) by Week Number of Year from Station e1248 on: "+ today)
